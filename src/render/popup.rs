...
pub(super) fn render_help_popup(f: &mut Frame, _app: &App) {
    let theme = app_theme();
    let area = centered_rect(53, 29, f.area());
    ...
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled("focus links; n/N next/prev", text_style),
        ]),
        Line::from(vec![
            Span::styled("Enter / o  ", key_style),
            Span::styled("copy / open HTTP(S) link", text_style),
        ]),
        Line::from(vec![
            Span::styled("f / Esc    ", key_style),
            Span::styled("exit link mode", text_style),
        ]),
        Line::from(""),
